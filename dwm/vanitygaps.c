...
/* broken tile layout for me nicksap*
static void
tile(Monitor *m)
{
        unsigned int i, n;
        ...
        for (i = 0, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++)
                if (i < m->nmaster) {
                        resize(c, mx, my, mw - (2*c->bw), (mh / mfacts) + (i <
mrest ? 1 : 0) - (2*c->bw), 0); my += HEIGHT(c) + ih; mfacts -= c->cfact; } else
{ resize(c, sx, sy, sw - (2*c->bw), (sh / sfacts) + ((i - m->nmaster) < srest ?
1 : 0) - (2*c->bw), 0); sy += HEIGHT(c) + ih; sfacts -= c->cfact;
                }
}
*/
