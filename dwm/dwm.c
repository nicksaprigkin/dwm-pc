...
void setdesktopnames(void) {
  XTextProperty text;
  char *tempTags[TAGSLENGTH];
  for (int i = 0; i < TAGSLENGTH; ++i) {
    tempTags[i] = strdup(tags[i]); // Duplicate the string
  }

  Xutf8TextListToTextProperty(dpy, tempTags, TAGSLENGTH, XUTF8StringStyle,
                              &text);
...
void drawbar(Monitor *m) {
  ...
  x = 0;
  for (i = 0; i < LENGTH(tags); i++) {
    // /* do not draw vacant tags(if you want this remove the occ commentrs
    // above too!) */ if (!(occ & 1 << i || m->tagset[m->seltags] & 1 << i))
    //   continue;
